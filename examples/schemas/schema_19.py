...
class BillingPeriod(BaseModel):
    start_date: date = Field(
        description="Start date of the billing period", examples=["2023-09-01"]
    )
    end_date: date = Field(
        description="End date of the billing period", examples=["2023-09-30"]
    )
...
class LineItem(BaseModel):
    ...
    line_item_occurrence_date: date = Field(
        description="Date when the service was performed", examples=["2023-09-15"]
    )
...
class MedicalInvoiceImageExtractionTaskOutput(BaseModel):
    ...
    creation_date: date = Field(
        description="Date when the invoice was generated", examples=["2023-10-01"]
    )
    ...
    invoice_identifier: str = Field(
        description="Unique identifier for the invoice", examples=["123456"]
    )
    line_items: List[LineItem] = Field(
        description="Array of line items on the invoice", min_length=1
    )
    total_net_amount: float = Field(
        description="Total amount due after discounts and deductions",
        examples=[150],
    )
    total_gross_amount: float = Field(
        description="Total amount due before discounts and deductions",
        examples=[165],
    )
