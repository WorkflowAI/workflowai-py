...
async def initial_translation_agent(
...
async def evaluate_translation_agent(
...
async def improve_translation_agent(
    input: TranslationImprovementInput
) -> TranslationImprovementOutput:
    """
    Expert literary translator.
...
async def translate_with_feedback(text: str, target_language: str) -> TranslationResult:
    ...
    while iterations < MAX_ITERATIONS:
        ...
        if (
            evaluation.quality_score >= 8 and
            evaluation.preserves_tone and
            evaluation.preserves_nuance and
            evaluation.culturally_accurate
        ):
            break
