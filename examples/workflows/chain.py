...
async def generate_marketing_copy_agent(
...
class EvaluateCopyOutput(BaseModel):
    ...
    hasCallToAction: bool = Field(description="Whether a call to action is present.")
    emotionalAppeal: int = Field(description="Emotional appeal rating (1-10).")
...
async def evaluate_marketing_copy_agent(
    input: EvaluateCopyInput
) -> EvaluateCopyOutput:
    """
...
async def rewrite_marketing_copy_agent(
    input: RewriteCopyInput
) -> RewriteCopyOutput:
    """
...
async def generate_marketing_copy(idea: str) -> MarketingResult:
    ...
    needs_improvement = (
        not evaluation.hasCallToAction
        or evaluation.emotionalAppeal < 7
        or evaluation.clarity < 7
    )
    ...
    if needs_improvement:
        rewrite = await rewrite_marketing_copy_agent(
            RewriteCopyInput(
                ...
                add_call_to_action=not evaluation.hasCallToAction,
                strengthen_emotional_appeal=evaluation.emotionalAppeal < 7,
...
if __name__ == "__main__":
    ...
    print(f"✓ Call to Action: {'Present' if metrics.hasCallToAction else 'Missing'}")
    print(f"✓ Emotional Appeal: {metrics.emotionalAppeal}/10")
